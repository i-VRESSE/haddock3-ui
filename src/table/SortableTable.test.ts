...
import { itemKeyFinder, sortData, type DataItem, type Header, type SortState } from "./SortableTable.js";
...
describe("sortData()", () => {
  ...
  test("should sort data by string in ascending order", () => {
    ...
    expect(sortedData.map(item => item.name)).toEqual(["Alice", "Bob", "Charlie"]);
  });

  test("should sort data by string in descending order", () => {
    const sortState: SortState = { key: "name", direction: "desc" };
    const sortedData = sortData(data, sortState, headers);
    expect(sortedData.map(item => item.name)).toEqual(["Charlie", "Bob", "Alice"]);
  });
  ...
  test("should sort data by stats mean in ascending order", () => {
    ...
    const sortedData = sortData(data, sortState, headers);
    expect(sortedData.map(item => (item.stats as { mean: number }).mean)).toEqual([5, 10, 15]);
  });

  test("should sort data by stats mean in descending order", () => {
    const sortState: SortState = { key: "stats", direction: "desc" };
    const sortedData = sortData(data, sortState, headers);
    expect(sortedData.map(item => (item.stats as { mean: number }).mean)).toEqual([15, 10, 5]);
  });
